    ...
    mainWindow.webContents.on('page-title-updated', async (e, title) => {
      ...
      if (!title.endsWith(armCordSuffix)) {
        ...
        await mainWindow.webContents.executeJavaScript(
          `document.title = '${
            (title.split(' | ')[1] ?? title) + armCordSuffix
          }'`
        );
      }
    });
    ...
    fs.readdirSync(themesFolder).forEach((file) => {
      try {
        const manifest = fs.readFileSync(
          `${themesFolder}/${file}/manifest.json`,
          'utf8'
        );
        ...
        if (
          ...
          console.log(
            `%cSkipped ${themeFile.name} made by ${themeFile.author}`,
            'color:red'
          );
        } else {
          mainWindow.webContents.send(
            'themeLoader',
            fs.readFileSync(
              `${themesFolder}/${file}/${themeFile.theme}`,
              'utf-8'
            )
          );
          console.log(
            `%cLoaded ${themeFile.name} made by ${themeFile.author}`,
            'color:red'
          );
        }
      } catch (err) {
