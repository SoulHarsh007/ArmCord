...
async function updateLang(): Promise<void> {
  ...
  if (parts.length === 2)
    ipcRenderer.send('setLang', parts.pop().split(';').shift());
}
declare global {
  interface Window {
    armcord: any;
...
sleep(5000).then(async () => {
  ...
  if (ipcRenderer.sendSync('disableAutogain')) {
    addScript(
      fs.readFileSync(
        path.join(__dirname, '../', '/content/js/disableAutogain.js'),
        'utf8'
      )
    );
  }
  addScript(
    fs.readFileSync(path.join(__dirname, '../', '/content/js/rpc.js'), 'utf8')
  );
