...
ipcRenderer.on('themeManifest', (_event, json) => {
  ...
  document.getElementById(`${id}header`)!.addEventListener('click', () => {
    ...
    document.getElementById('themeInfoName')!.textContent =
      `${manifest.name} by ${manifest.author}`;
    document.getElementById('themeInfoDesc')!.textContent =
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('download')!.addEventListener('click', () => {
    ipcRenderer.send(
      'installBDTheme',
      (document.getElementById('themeLink') as HTMLInputElement)!.value
    );
  });
});
